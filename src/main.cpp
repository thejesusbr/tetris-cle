...
unsigned char *pField =
    nullptr;             // The gaming fiels will be stored as a char array
...
WINDOW *screen,          // Windows used to display stuff
    *scoreScreen, *nextPieceScreen, *specialScreen;
int ch; // Variable to manage user input

// HELPER FUNCTIONS
// ===================================================================

// To apply rotation to a piece, instead of rotating the piece itself, we
// translate the coordinates. This way we can easily create a new piece without
// having to manage each piece alone, saving some precious memory
...
// Collision detection. A rather simple proceeding, we just compare the position
// in the tetromino with the position it'll occupy in the field. If the
// tetromino has an 'X' and the field has anything but a blank, then it will
// collide and thus block the piece in place
...
int main(int argc, char **argv) {
  ...
  int nCurrentPiece =
      distribution(generator); // rand() % 7;      // We initialize the current
                               // piece randomly
  int nNextPiece = distribution(
      generator); // rand() % 7;         // And also the next piece as well
  int nCurrentRotation = 0; // All pieces start in their unrotated state
  int nCurrentX =
      nFieldWidth / 2 - 2; // This will put the piece centered in the field
  int nCurrentY =
      0; // And at the first line. The (0,0) of the piece is the upper left cell

  int nSpeed =
      60; // The initial game speed. Decreasing this will make the game harder
  int nSpeedCounter =
      0; // A counter to increase the speed at determined moments
  uint nPieceCount =
      0; // A counter to store how many pieces were put on the field

  bool bForceDown = false; // A control variable to force the piece to move

  bool bRotateHold = false; // A control variable to block continuous rotation
  ...
  uint nSpecial =
      0; // 'Swap' special power, allows you to swap current for next piece
  uint nRest = 0;
  uint nScoreCounter = 0;

  std::chrono::milliseconds gameTick(
      16); // The game ticks at 16ms (roughly 60fps)
  std::chrono::milliseconds lineWait(
      400); // A pause of roughly half a second to display a line removal
            // animation
  ...
  /* The game main loop is divided in sections:
     TIMING: here we take care of timing related routines. Remember each
     computer is different, so we need to standardize how we account the time.
     The chrono namespace has a lot of good utilities for this. INPUT:  this
     section manages the user input. As this game is very simple, using an event
             driven approach would be overkill, so we'll use good ol' polling.
     GAME LOGIC: in this section, we manage to do all the calculations for
     changing the game state each frame. RENDER: finally, we concentrate drawing
     related stuff here. This approach is the most used by a variety of games.
     Obviously, sometimes we have to brake the hierarchy and perform game logic
     in the render section or vice-versa, depending of what we need to do. But
     for most of time, we can just follow the structure.
  */
  while (!bGameOver) {
    ...
    if (!bPaused) {
      ...
      bForceDown = (nSpeedCounter ==
                    nSpeed); /* When we have as many ticks as the current
    ...
    ch = getch(); // Which key the player pressed in this tick? (non-blocking
                  // and buffered)
    ...
    case 'Q': // The player wants to quit. Case insensitive
      bGameOver = true;
      break;
    case KEY_LEFT: // Left arrow pressed, check collision and move the piece if
                   // possible
      if (DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX - 1,
                       nCurrentY)) {
        --nCurrentX;
      }
      break;
    ...
    case KEY_DOWN: // Down arrow pressed
      ...
      break;
    case 'z':
    case 'Z': // Z key is used for rotation. Here, we also block the movement if
              // the player holds down the key, so it will always rotate once
      nCurrentRotation +=
      ...
      bRotateHold = true;
      break;
    case ' ': // Space key uses the special power, that swaps current and next
              // piece
      if (nSpecial > 0) {
        nSpecial--;
        temp = nCurrentPiece;
    ...
    if (bForceDown) {
      ...
      if (DoesPieceFit(nCurrentPiece, nCurrentRotation, nCurrentX,
        ...
        // Check for full lines. Here, we don't need to check the entire field,
        // we just check within the last piece played. We run over its height,
        // checking if every cell along the width is not blank. The full line is
        // filled with a different charater and the Y coordinate is pushed back
        // to the vector
        for (int py = 0; py < 4; py++) {
        ...
        // Removing lines gives 2^n * 100 points, so the more lines removed,
        // greater the reward
        if (!vLines.empty())
        ...
        nNextPiece =
            distribution(generator); // rand() % 7;    // And we sort a new one
    ...
    for (uint x = 0; x < nFieldWidth; ++x) {
      for (uint y = 0; y < nFieldHeight; ++y) {
        ...
        // We draw a character based in the field coding. Here is a neat trick,
        // we chose the corresponding character based on the code. This helps to
        // save some memory and cycles
    ...
    // Draw current piece. Here, we iterate over the current tetromino shape and
    // check for filled cells Before checking, we need to apply the rotation.
    wset_color(screen, PIECE_COLOR);
    for (int px = 0; px < 4; ++px) {
      for (int py = 0; py < 4; ++py) {
        if (tetromino[nCurrentPiece][Rotate(px, py, nCurrentRotation)] == 'X') {
          mvwprintw(screen, (nCurrentY + py + 2), (nCurrentX + px + 2), "%c",
                    nCurrentPiece + 65); // Nice trick :-)
        }
      }
    }
    wunset_color(screen, PIECE_COLOR);
    ...
    // Draw score
    int nDigits =
        (int)log10(nScore) + 1; // Trick to count how many digits the score has
    mvwprintw(scoreScreen, 1, 11 - nDigits, "%d",
              nScore); // The alignment is adjusted for the number of digits
    ...
    if (!vLines.empty()) {
      ...
      std::this_thread::sleep_for(
          lineWait); // We want to show it for about half a second

      // Pop lines and move down
      for (auto &l : vLines) { // Running over the stored coordinates
        for (int px = 1; px < nFieldWidth - 1;
             px++) { // and then, over the corresponding columns
          for (int py = l; py > 0; py--) {
            pField[py * nFieldWidth + px] =
                pField[(py - 1) * nFieldWidth +
                       px]; // and movin everything down
          }
          pField[px] = 0;
        }
      }
